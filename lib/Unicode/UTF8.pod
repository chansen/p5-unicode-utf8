...
unsigned integer containing the octet position at which the error occurred 
within the octets provided to C<decode_utf8()>. C<$fallback> must return a 
character string consisting of zero or more Unicode scalar values. Unicode 
scalar values consist of code points in the range U+0000..U+D7FF and 
U+E000..U+10FFFF.
...
C<$position> is a unsigned integer containing the character position at which 
the error occurred within the string provided to C<encode_utf8()>. C<$fallback> 
must return a character string consisting of zero or more Unicode scalar values.
