...
Throws an exception if C<$octets> contains an ill-formed UTF-8 sequence or a code 
point which can't be interchanged.
...
Throws an exception if C<$string> contains a code point which can't be interchanged
...
=item Can't decode ill-formed %s octet sequence <%s> in position %u
...
=item Can't represent surrogate code point U+%.4X in UTF-8 encoding form
...
=item Can't represent restricted code point U-%.8X in UTF-8 encoding form

(F) Code points in the range U-00110000 to U-7FFFFFFF.

ISO/IEC 10646 originally defined codespace up to U-7FFFFFFF. This was 
restricted by JTC1/SC2/WG2 Resolution M38.6 (Restriction of encoding space)
to U-0010FFFF in 2000.

=item Can't represent super code point \x{%X} in UTF-8 encoding form
