...
Here is a summary of features for comparison with L<Encode>:

=over 4

=item *

simple API which integrates with warnings category C<utf8>
...
Issues a warning using L<warnings> category C<utf8> if C<$octets> contains 
ill-formed UTF-8 sequences or encoded code points which can't be interchanged.

<$fallback> is a C<CODE> reference which provides a error-handling mechanism, 
...
C<$fallback> is invoked with two arguments, C<$octets> and <$is_usv>. C<$octets>
constains a sequence of one or more octets containing the maximal subpart of 
the ill-formed subsequence or encoded code point which can't be interchanged.
C<$is_usv> is a boolean indicating whether or not C<$octets> represent a 
encoded Unicode scalar value. C<$fallback> must return a character string 
consisting of zero or more characters.
...
Issues a warning using L<warnings> category C<utf8> if C<$string> contains 
...
C<$fallback> is invoked with two arguments, C<$codepoint> and <$is_usv>. 
C<$codepoint> is a unsigned integer containing the code point which can't 
represented in UTF-8 encoding form or be interchanged. C<$is_usv> is a boolean 
indicating whether or not C<$codepoint> is a Unicode scalar value. C<$fallback> 
must return a character string consisting of zero or more characters.
...
(W utf8) Noncharacters is permanently reserved for internal use and that
...
(W utf8) Surrogate code points are designated only for surrogate code units in the 
...
Please report any bugs or feature requests by email to C<bug-unicode-utf8 at rt.cpan.org>, or through
the web interface at L<http://rt.cpan.org/Public/Dist/Display.html?Name=Unicode-UTF8>. You will be automatically notified of any
